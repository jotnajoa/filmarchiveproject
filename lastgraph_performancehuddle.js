...
export async function loadData() {
    ...
    /* this part is for animation effect
...
function scaleGenerator(type, input, max) {
    ...
    let output = `<svg class='graphbody'>
                    ...
                    d="M0 50 C10 ${upvalue} 40 ${upvalue} 50 50 C40 ${downvalue} 10 ${downvalue} 0 50 z"/>
                  </svg>`
...
const showBack = () => {
    ...
    svg.selectAll('.mapoutline').remove()
...
//     .style('opacity', 0.8)
//     .attr('cx', (d) => {
//         return Math.random() * (850 - 257) + 257
//     })
//     .attr('cy', (d) => {
...
//         types.forEach((t) => {
//             if (d.type == t) {
//                 targettype = t.replace(/ /g, "")
//             }
//         })
//         if (targettype) {

//             return $(`.${targettype}`).position().top - 80
//         }
//     })
...
function moveBack() {
    ...
    svg.selectAll('circle')
        .transition().duration(1000)
        .attr('cx', (d, i) => {
            return minifiedData[i].lastcx
        })
        .attr('cy', (d, i) => {
            return minifiedData[i].lastcy
        })

}

function coordinateCal() {
    minifiedData.forEach((d) => {
        d.lastcx = Math.random() * (850 - 257) + 257
        ...
        types.forEach((t) => {
            ...
            if (targettype) {
                d.lastcy = $(`.${targettype}`).position().top - 80
            }
